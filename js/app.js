...
document.addEventListener('DOMContentLoaded', () => {
  ...
  aiSubmitBtn.addEventListener('click', () => {
    generateAIComment(`다음 텍스트를 바탕으로 인터넷 방송 시청자가 방장(스트리머)에게 건네는 채팅 멘트로 다듬어주세요. [중요: 당신은 방송을 시청하는 '시청자'이고 대화 상대는 '방장'입니다!] 유튜브 방송 분위기에 맞게, 감성이 듬뿍 담긴 이모지를 팍팍 섞어서 1~2문장의 부드럽고 유쾌한 멘트로 만들어주세요. 반드시 결과 멘트만 출력하세요.`);
  });

  if (aiTeaseBtn) {
    aiTeaseBtn.addEventListener('click', () => {
      generateAIComment(`다음 텍스트를 바탕으로 인터넷 방송 시청자가 방장(스트리머)을 장난스럽고 유쾌하게 놀리거나 팩트 폭행을 하는 채팅 멘트로 다듬어주세요. [중요: 당신은 방송을 시청하는 '시청자'이고 놀림의 대상은 '방장'입니다!] 악플이 되지 않도록 선을 지키며, 장난스러운 이모지(😜, 🤣, 👀 등)를 적극 사용하세요. 1~2문장으로 작성하고 반드시 결과 멘트만 출력하세요.`);
    });
  }
